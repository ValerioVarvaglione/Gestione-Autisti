...
const ServiceVeicoli = require('../services/ServiceVeicoli');
...
// Endpoint per ottenere veicoli con assicurazione scaduta
router.post('/assicurazione/scaduta', async (req, res) => {
  try {
    // Ottieni la data odierna
    const oggi = new Date();

    // Trova tutti i veicoli con assicurazione scaduta
    const veicoliScaduti = await serviceVeicoli.getVeicoliConAssicurazioneScaduta();
console.log(veicoliScaduti);
    // Controlla se ci sono veicoli con assicurazione scaduta
    if (!veicoliScaduti || veicoliScaduti.length === 0) {
      return res.status(404).json({ message: 'Nessun veicolo con assicurazione scaduta trovato.' });
    }

    // Invia i veicoli con assicurazione scaduta come risposta
    res.status(200).json(veicoliScaduti);
  } catch (error) {
    console.error('Errore nel recupero dei veicoli scaduti:', error);
    res.status(500).json({ message: 'Errore nel recupero dei veicoli scaduti.', error: error.message });
  }
});
